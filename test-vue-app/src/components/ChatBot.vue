<template>
    <div>
        <!-- transition: 过渡动画组件，用于为元素的显示/隐藏、进入/离开等状态变化添加动画效果 -->
        <transition name="rec-in-left">
            ...
            <div class="recognize" v-show="multishow">
                ...
                <div class="answer">
                    ...
                    <!-- class指的都是CSS类名， ref指的是变量直接访问该 DOM 元素 -->
                    ...
                    <!-- 点击事件 切换布尔值 multishow 动画滑入、滑出 -->
                </div>
            </div>            
        </transition>
        
        <!-- outer这个容器才是聊天界面的主题框框 -->
        <div class="outer">
            <div class="header">
            ...
            <div class="show1" v-show="!isshow">
                <div class="main" ref="dialogueContainer">
                    <div class="screen-inner">
                        <!-- Vue组件的data函数中，通常会定义组件的数据属性,所以想要查看messages数据定义要去script部分查看 -->
                        <!-- messages是组件的数据属性，而message是模板中循环时的临时变量。需要明确区分两者的作用域和定义位置。 -->
                        <div v-for="(message,index) in messages" :key="index" :class="message.sender==='robot'?'robot-dialogue':'man-dialogue'">
                            <!-- message临时变量循环数组messages,判断message的sender是否是robot，进而采取不一样的css样式 -->
...
<script>
    export default {
        name:'ChatBot',
        // 这边data可以直接理解为我这一个vue界面所需要的所有数据变量
        data(){
            return{
                notedata:'',
                multiword:'',
                ...
                // 要实现动态的将对话放置到聊天界面中，那么就要定义一个数组messages，里面定义两个变量，一个为text，用于存储文本，另一个为sender，
                // 该属性用于实现机器人和用户的样式转换，若当前值为robot，则样式class动态绑定robot；若为man，则绑定用户样式。
                messages:[
